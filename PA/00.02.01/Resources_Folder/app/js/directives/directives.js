(function() {
  ...
  app.directive('appMenu', function($fum, $timeout) {
    return {
      ...
      replace: true,
      restrict: 'E',
      templateUrl: "" + $fum.modulePath + "/app/partials/vertical-menu.html"
    };
  });
  ...
  app.directive('opsiUserDetail', function($fum) {
    return {
      ...
      templateUrl: "" + $fum.modulePath + "/app/partials/user-detail.html"
    };
  });
  ...
  app.directive('opsiUserTiles', function($fum) {
    return {
      ...
      replace: true,
      restrict: 'E',
      templateUrl: "" + $fum.modulePath + "/app/partials/user-tiles.html"
    };
  });
  ...
  $opsiUserFiltersTemplate = '<div class="app-pager"> <div class="item"> <select class="page-filter" data-ng-model="orderby" data-ng-change="changePage()"> <option value="displayname">Display name</option> <option value="email">Email</option> <option value="lastlogindate">Last login date</option> <option value="lastname">Lastname</option> <option value="name">Name</option> <option value="regdate">Registration date</option> <option value="userid">Userid</option> <option value="username">Username</option> </select> <div class="page-filter"> <span data-ng-show="orderclause == \'ASC\'" data-ng-click="orderclause = \'DESC\'; changePage()"><i class="icon-down-circled"></i> Ascending</span> <span data-ng-show="orderclause == \'DESC\'" data-ng-click="orderclause = \'ASC\'; changePage()"><i class="icon-up-circled"></i> Descending</span> </div> </div> <div class="item"></div> <div class="item users-count right"> <strong>{{userCount}}</strong> {{translate.registeredUsers}} - {{translate.browsingPage}} {{pageNumber}} {{translate.browsingPageOf}} {{pages.length}}. </div> <div class="item"> <input class="page-select" type="number" min="1" max="{{pages.length}}" data-ng-model="pageNumber" data-ng-change="changePage();" placeholder="Write page"> </div> </div>';
